...
func NewSandbox(key string, osCreate, isRestore bool) (Sandbox, error) {
	...
	if err != nil {
		log.Warnf("Failed to disable IPv6 on all interfaces on network namespace %q: %v", n.path, err)
	}
...
func (n *networkNamespace) checkLoV6() {
	...
	if err := setIPv6(n.path, "lo", enable); err != nil {
		log.Warnf("Failed to %s IPv6 on loopback interface on network namespace %q: %v", action, n.path, err)
	}
...
func reexecSetIPv6() {
	...
	if len(os.Args) < 3 {
		log.Errorf("invalid number of arguments for %s", os.Args[0])
		os.Exit(1)
	}
	...
	if err = netns.Set(ns); err != nil {
		log.Errorf("setting into container netns %q failed: %v", os.Args[1], err)
		os.Exit(3)
	}
	...
	if err = ioutil.WriteFile(path, []byte{value, '\n'}, 0644); err != nil {
		log.Errorf("failed to %s IPv6 forwarding for container's interface %s: %v", action, os.Args[2], err)
		os.Exit(4)
	}
