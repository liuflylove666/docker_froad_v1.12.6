...
for version in "${versions[@]}"; do
	...
	case "$from" in
		...
		centos:7)
			...
			echo 'RUN [ `rpm -q selinux-policy-devel | grep el7_3` ] || yum -y  install selinux-policy-devel' >> "$version/Dockerfile"
			;;
		*)
			echo "RUN ${installer} install -y ${packages[*]}" >> "$version/Dockerfile"
			;;
	...
	echo 'RUN curl -fSL "https://dl.gocn.io/golang/${GO_VERSION}/go${GO_VERSION}.linux-amd64.tar.gz" | tar xzC /usr/local' >> "$version/Dockerfile"
